...
def _run_ffmpeg_pass(in_path: str, out_path: str, bitrate: str) -> bytes | None:
    """Run ffmpeg pass and return the generated bytes if successful."""
    cmd = [
        "ffmpeg",
        "-y",
        "-i",
        in_path,
        "-vf",
        "scale='if(gt(iw,ih),512,-2)':'if(gt(iw,ih),-2,512)',fps=30",
        "-c:v",
        "libvpx-vp9",
        "-b:v",
        bitrate,
...
def convert_video_to_sticker(file_bytes: io.BytesIO) -> io.BytesIO | None:
    ...
    try:
        ...
        data = _run_ffmpeg_pass(tmp_in_path, tmp_out_path, "200k")

        if data and len(data) > 256_000:
            os.unlink(tmp_out_path)
            tmp_out_path2 = tmp_in_path.replace(".mp4", "_out2.webm")
            data2 = _run_ffmpeg_pass(tmp_in_path, tmp_out_path2, "100k")
            if data2:
                data = data2
