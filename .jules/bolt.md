...
## 2024-06-28 - Image processing optimization in PIL
**Learning:** For PIL operations, converting mode (e.g. to RGBA) before resizing a large image is noticeably slower than resizing first and converting mode after. Thumbnail does not always yield better performance than `resize` for PNGs, but resizing before converting color space saves memory and CPU time during the resize operation. However, Pillow cannot apply continuous mathematical resampling filters (like `LANCZOS`) to paletted/indexed images (mode `"P"`, which is extremely common for optimized web PNGs) or binary images (mode `"1"`). If `resize()` is called on an image with mode `"P"`, Pillow silently ignores the `LANCZOS` argument and falls back to `NEAREST` neighbor interpolation.
**Action:** When handling image processing in `src/stickers/media.py` (both `apply_mask_to_image` and `convert_to_sticker`), ALWAYS make sure images are converted to `"RGBA"` (or `"L"` for masks) *before* applying the `LANCZOS` resize to ensure consistent, anti-aliased interpolation regardless of the original file's internal color mode.
