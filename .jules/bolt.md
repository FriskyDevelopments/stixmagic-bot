...
## 2024-05-23 - Mock Environment Variable Loading
**Learning:** Found that tests importing `infra.db` without mocking `sys.modules['dotenv']` would fail because the application attempts to load `dotenv` which is an optional dependency or might not exist in the test environment, causing `ModuleNotFoundError`.
**Action:** When mocking environment or application variables that load third-party libraries globally during import (like `stixmagic.settings` loading `dotenv`), explicitly stub out the third-party library in `sys.modules` *before* importing the module under test. This allows tests to run successfully without bringing in the actual external dependency and causing test environment pollution.
