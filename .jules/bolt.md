...
## 2026-05-08 - Batched Database Operations
**Learning:** Found that `api.py` was issuing individual `UPDATE` and `DELETE` queries within a loop when validating sticker packs against Telegram via `_validate_packs_async`. This triggered multiple round-trips to the database when processing multiple packs, reducing total execution time for bulk operations.
**Action:** Always use `executemany()` to batch database operations when multiple rows need to be updated or deleted. This optimizes performance by minimizing database round-trips and leveraging SQLite's prepared statement reuse. When verifying these operations in tests, verify calls against `cursor.executemany.call_args_list` rather than `cursor.execute.call_args_list`.
