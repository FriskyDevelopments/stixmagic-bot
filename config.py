...
Environment is controlled via the APP_ENV environment variable:
  ...
  Only these exact values are allowed. Any other value causes an immediate startup failure.
...
# Only the two canonical environment values are allowed (no aliases).
_ALLOWED_ENVS: frozenset[str] = frozenset({"development", "production"})

if _RAW_ENV not in _ALLOWED_ENVS:
    _allowed = ", ".join(repr(k) for k in sorted(_ALLOWED_ENVS))
    sys.stderr.write(
...
def validate_config() -> None:
    ...
    # 2. Production safety: reject if DEV_BOT_TOKEN is present.
    if IS_PRODUCTION:
        ...
        if dev_raw:
            errors.append(
                ...
                "Unset DEV_BOT_TOKEN or change APP_ENV."
            )

    # 3. Development safety: reject if TELEGRAM_BOT_TOKEN is present.
    if IS_DEVELOPMENT:
        ...
        if prod_raw:
            errors.append(
                ...
                "Unset TELEGRAM_BOT_TOKEN or change APP_ENV."
            )
